#!/usr/bin/env python3
r"""
Fig 9 — The suprahyoid sensitivity field for a retroauricular montage.
...
`assert_anonymised()`.
    ...
    ~/Applications/SimNIBS-4.6/bin/simnibs_python figures/render_fig9.py
"""
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(prog="render_fig9.py")
    ...
    def slab(axis, value, half, u, v):
        ...
        solid = _nd.uniform_filter(
            (~np.isnan(img)).astype(float), size=3) >= 0.78
        # Also drop everything not connected to the head. The neighbourhood
        # filter cleans the interior but leaves a rim of detached pixels where
        # the slab grazes the scalp tangentially, which reads as noise around
        # an otherwise clean outline.
    ...
    rc.assert_anonymised("fig9_suprahyoid_field", True)
    if not rc.PAPER_MODE:
        fig.suptitle("Fig 9 · Suprahyoid sensitivity field, retroauricular montage",
    ...
    rc.save(fig, "fig9_suprahyoid_field", prov, a.outdir)
    return 0
