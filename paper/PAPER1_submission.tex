...
\makeatother
\hypersetup{
  pdftitle={No speech articulator robustly favours retroauricular electrodes over canonical jaw sites: a volume-conductor model with orientation and electrode-count controls},
  pdfauthor={Carl Vincent Kho \textbackslash\textbackslash{} Independent researcher; Somach Systems, Inc., San Francisco, CA, USA \textbackslash\textbackslash{} carl@somach.life \textbackslash textperiodcentered\{\} https://somach.life},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\author{Carl Vincent Kho \textbackslash\textbackslash{} Independent researcher; Somach Systems, Inc., San Francisco, CA, USA \textbackslash\textbackslash{} carl@somach.life \textbackslash textperiodcentered\{\} https://somach.life}
\date{}
...
\textbf{Objective.} Ear-worn biopotential devices are being designed around a
...
\textbf{Approach.} Articulator-to-electrode coupling is computed by reciprocity on
MIDA, a head model with 116 labelled compartments at 500 \(\mu\)m, treating muscle as
...
\textbf{Main results.} The two montages see different muscles, but the effect is
...
subsample. \textbf{No muscle robustly favours the retroauricular montage.} The
...
\textbf{Significance.} For device design the result is one-sided rather than a
...
10.13099/ViP-MIDA-V1.0), segmented at 500 \(\mu\)m isotropic. The ten segmented
...
246,872 mm\(^{3}\)) and \texttt{Tongue} (label 42; 521,131 voxels, 65,130 mm\(^{3}\)). They are
...
−0.99892{]}, tilted 2.664\(^{\circ}\) off the RAS S axis, passing through (−2.301, 23.112,
...
{[}0.03335, 0.03240, 0.99892{]} at 2.665\(^{\circ}\) from RAS +S with a 0.500 mm step, agreeing
with the fitted normal to 0.0026\(^{\circ}\). The truncation therefore lies on a voxel plane
...
\textbf{Air is a numerical choice, not a physical one.} True air conductivity is
...
\(\sigma\)\_air = 1 \(\times\) 10\(^{-15}\) S/m the span reaches 1.879 \(\times\) 10\(^{15}\), the iterative solve does
not converge, and the returned fields are 10--20\(\times\) too large while a result file
is still written. We use \(\sigma\)\_air = 1 \(\times\) 10\(^{-6}\) S/m throughout, giving a span of
1.879 \(\times\) 10\(^{6}\). A pre-flight gate refuses any assignment whose span exceeds
1 \(\times\) 10\(^{8}\).

Insensitivity to that choice was measured rather than assumed. Solving at
\(\sigma\)\_air = 10\(^{-6}\), 10\(^{-5}\), 10\(^{-4}\) and 10\(^{-3}\) S/m on identical geometry, the largest
departure over the ten muscle compartments from the 10\(^{-6}\) baseline is 0.007 dB at
10\(^{-5}\), 0.069 dB at 10\(^{-4}\) and 0.467 dB at 10\(^{-3}\). The result is therefore
insensitive across 10\(^{-6}\) to 10\(^{-4}\), where the largest departure sits roughly four
times below the measured noise floor of 0.27 dB. Departure begins at 10\(^{-3}\), where
it reaches 1.7\(\times\) the floor and concentrates in the compartments nearest the oral
...
source. For a current dipole at position \textbf{r} with moment \textbf{p} and a recording
pair (A, B), the measured potential difference is

\begin{verbatim}
V_AB(r, p) = E_recip(r) \textperiodcentered{} p / I
\end{verbatim}
...
source --- 22 solves against the order of 10\(^{5}\) a forward formulation would require
...
are taken at the pre-registered cluster {[}\texttt{results/04q\_table4\_envelope.csv},
\texttt{results/04k\_fan\_fractions.csv}{]}; the same fan read against all fourteen ear sites
...
L(A\(\rightarrow\)B) = −L(B\(\rightarrow\)A). On identical discretisations the residual is 7.5 \(\times\) 10\(^{-6}\) of
\textbar L\textbar, i.e.~6.5 \(\times\) 10\(^{-5}\) dB --- four orders of magnitude below the measured per-site
noise floor.
...
orientation are exact by construction (the patch surface closes to 1.2 \(\times\) 10\(^{-16}\)
...
Everything is reproducible from a clean checkout given MIDA in \texttt{data/}; the one
...
\texttt{fa583f6}, dated 2026-08-02. The lead-field results that test that prediction
...
\textbf{A published table with no generating script is not a result yet.} Two tables
in this study (\texttt{04h\_matched\_counts.csv}, \texttt{04j\_two\_axis\_verdict.csv}) were
initially produced interactively rather than by a script in \texttt{src/}. They could
...
every number in it is internally consistent whether or not it is right. Both are now generated by \texttt{src/04h\_matched\_counts.py}, and no
quantity enters the manuscript except from a file some script in \texttt{src/} writes.

\textbf{The prediction was not confirmed, and that is reported here rather than
...
\textbf{The jaw's dominance over the labial group is robust on both axes.}
Orbicularis oris, buccinator, mentalis, depressor anguli oris and platysma
...
\textbf{No articulator favours the ear on both axes.} Temporalis is the closest,
...
\textbf{{[}−1.453, +5.458{]}} and only \textbf{50.2 per cent} of random four-site
...
\textbf{The two treatments disagree, and the anatomy-specific one governs.} Under a
...
\textbf{Two show no preference that survives electrode subsampling.}
...
\textbf{Two favour the jaw robustly across sites but not across orientation.}
...
\textbf{The isotropy assumption does not measurably affect any site-to-site ratio}
(Figure 4).
...
\textbf{The mechanism is therefore geometric rather than material.} Removing the
...
movements are \texttt{medial\_pterygoid} −0.88, \texttt{sternocleidomastoid} −0.54 and
\texttt{platysma} −0.37 dB. The reported advantage is not an artefact of proximity to
...
\textbf{A homogeneous soft-tissue conductor reproduces every montage assignment.}
...
\textbf{Placement chosen by anatomical target outperforms arbitrary placement.} The
...
it: \textbf{the claim that no articulator favours the retroauricular montage rests on
...
layer profile exists, it correlates \emph{negatively} with the adipose fraction of
the muscle-to-skin path (Spearman \(\rho\) = −0.955, p = 0.001, n = 7): platysma sits
...
share is \textbar{}\(\Delta\)gap\textbar{} / \textbar gap\textbar, and a muscle embedded uniformly in fat has both the
jaw and the ear route shifted together, so the change subtracts out of the
ratio. The quantity that would track positively is the \emph{difference} between
...
pterygoid --- and their best positions differ, so contamination at \texttt{cg01} is not
the same mixture as contamination at \texttt{cg08}.
...
derived per-voxel fibre field & \textbf{−1.15 dB, interval {[}−1.45, +5.46{]} spans zero} \\
\end{longtable}
}
...
the \textbf{final} step, the one that crosses zero, removes an assumption rather than
...
\textbf{Fibre orientation is bounded, not known} (§2.5). A fibre tensor reaches two
...
untested here. \textbf{Orbicularis oris admits it in no form}: it is a sphincter whose
...
distance to the nearest electrode. Sorted by volume fraction \(\times\) proximity.
{[}\texttt{results/table1\_conductivities.csv}{]}
...
4 & Muscle anisotropy & \(\sigma\) tensor vs scalar & \textbf{yes --- \textasciitilde5 dB in medial pterygoid, \textasciitilde4.5 dB in SCM} & \textbf{no --- below the floor} & statistic A: largest change to any gap is \textbf{−0.085 dB} (SCM); medial pterygoid −0.010, temporalis +0.137, lateral pterygoid +0.036, all under the 0.27 dB floor. The absolute lead field IS affected; the reclassification is specific to ratios. Tensor on 2 of 10 compartments, the rest NOT APPLIED \\
...
8 & Delivered current & injected vs requested per solve & yes & \textbf{no --- corrected, not bounded} & 0.887--1.075 \(\times\) requested across 22 solves, measured per solve by the tet-patch integral. Each site's lead field is divided by its own delivered current (§2.4), so the term does not enter any reported ratio. It is listed here because it was measured and corrected, not because it remains an uncertainty: the 1.67 dB spread it would otherwise contribute is six times the row-6 floor and could not have been bounded by it \\
9 & Adipose conductivity & fat at 0.025 vs muscle 0.355 S/m & yes & \textbf{yes --- and the SIGN differs by muscle} & Reported \textbf{per muscle only} --- a population differential across sites has no clean definition under statistic A (the median change over muscles is +0.01 dB and conceals a sign that spans −2.86 to +1.09). Statistic A, per muscle, sign varies: \textbf{−1.121 dB} for temporalis (acts against the gap), \textbf{+0.411 dB} for sternocleidomastoid and \textbf{+0.323 dB} for lateral pterygoid (act with it), \textbf{−2.863 to +1.093 dB} across the labial group. No single figure is admissible --- the sign differs by muscle. Shares are not quoted here because they are dominated by the denominator; see §4.3. \\
\end{longtable}
}
...
2.8 \(\times\) 10\(^{-14}\) mm) so the exact answer is identical across draws and the whole
...
sternocleidomastoid & -2.53 to -0.97 & 1 of 5 & 60.5--65.0 \% & \textbf{unstable across subsets} \\
lateral pterygoid & -3.61 to -1.53 & 1 of 5 & 65.5--72.0 \% & \textbf{unstable across subsets} \\
temporalis & -5.42 to -2.57 & yes, all 5 & 92.0--100.0 \% & \textbf{ear, robust on both axes} \\
\end{longtable}
}
...
advantage. The subset that deviates is the one dropping \texttt{midjaw}, which has the
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{/Users/carl/CODELocalProjects/ear-emg-forward-model/figures/fig1_head_model.pdf}
\caption{MIDA head model with the ten segmented articulator compartments highlighted, and all 22 electrode positions shown in lateral, frontal and posterior views. The face is masked above the orbital rim in accordance with the MIDA licence.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{/Users/carl/CODELocalProjects/ear-emg-forward-model/figures/fig3_attenuation_vs_distance.pdf}
\caption{Attenuation against source depth for each articulator, jaw sites against retroauricular sites.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{/Users/carl/CODELocalProjects/ear-emg-forward-model/figures/fig5_complementarity_map.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{/Users/carl/CODELocalProjects/ear-emg-forward-model/figures/fig6_suprahyoid_field.pdf}
...
tables are available at \texttt{github.com/Somach-Systems-Inc/ear-emg-forward-model}.
The MIDA model itself cannot be redistributed under its licence and must be
obtained from the IT'IS Foundation; \texttt{REPRODUCTION.md} documents the one manual
step. Commits \texttt{fa583f6} (2026-08-02) and the lead-field commit (2026-08-03) are
...
\tightlist
\item
  An, et al.~(2025). ID.EARS. \emph{CHI '25}.
\item
  Avramidou, et al.~(2024). From Ear-EEG to Ear-ExG: The Jaw Artifact is a Keeper. \emph{DSAI '24}.
\item
  Debener, S., et al.~(2015). Unobtrusive ambulatory EEG using a smartphone and flexible printed electrodes around the ear. \emph{Sci Rep} 5:16743.
\item
  De Luca, C. J., et al.~(2011). Inter-electrode spacing of surface EMG sensors. \emph{J Biomech}.
\item
  Gaddy, D., \& Klein, D. (2020). Digital Voicing of Silent Speech. \emph{EMNLP}.
\item
  Goncharova, I. I., et al.~(2003). EMG contamination of EEG: spectral and topographical characteristics. \emph{Clin Neurophysiol}.
\item
  Harmening, N., Klug, M., Gramann, K., \& Miklody, D. (2022). HArtMuT --- modeling eye and muscle contributors in neuroelectric imaging. \emph{J Neural Eng} 19(6):066041. doi:10.1088/1741-2552/aca8ce
\item
  Iacono, M. I., et al.~(2015). MIDA: A Multimodal Imaging-Based Detailed Anatomical Model of the Human Head and Neck. \emph{PLOS ONE}. doi:10.1371/journal.pone.0124126
\item
  Kappel, S. L., Makeig, S., \& Kidmose, P. (2019). Ear-EEG Forward Models: Improved Head-Models for Ear-EEG. \emph{Front Neurosci} 13:943. doi:10.3389/fnins.2019.00943
\item
  Kapur, A., Kapur, S., \& Maes, P. (2018). AlterEgo: A Personalized Wearable Silent Speech Interface. \emph{IUI '18}.
\item
  Kuiken, T. A., Lowery, M. M., \& Stoykov, N. S. (2003). The effect of subcutaneous fat on myoelectric signal amplitude and cross-talk. \emph{Prosthet Orthot Int} 27(1):48--54. doi:10.3109/03093640309167976
\item
  Maksymenko, K., Deslauriers-Gauthier, S., \& Farina, D. (2021). Ultra fast and highly realistic numerical modelling of surface EMG. \emph{bioRxiv}.
\item
  Meiser, A., Knoll, J., \& Bleichner, M. G. (2024). High-density ear-EEG for understanding ear-centered EEG. \emph{J Neural Eng} 21(1):016001. doi:10.1088/1741-2552/ad1783
\item
  Mesin, L. (2020). Crosstalk in surface electromyogram: literature review. \emph{Phys Eng Sci Med}. doi:10.1007/s13246-020-00868-1
\item
  Sato, W., \& Kochiyama, T. (2023). Crosstalk in Facial EMG and Its Reduction Using ICA. \emph{Sensors} 23:2720.
\item
  Saturnino, G. B., Madsen, K. H., \& Thielscher, A. (2019). Electric field simulations for transcranial brain stimulation using FEM: an efficient implementation and error analysis. \emph{J Neural Eng} 16(6):066032. doi:10.1088/1741-2552/ab41ba
\item
  Thielscher, A., Antunes, A., \& Saturnino, G. B. (2015). Field modeling for transcranial magnetic stimulation: a useful tool to understand the physiological effects of TMS? \emph{37th Annual International Conference of the IEEE Engineering in Medicine and Biology Society (EMBC)}, 222--225. doi:10.1109/EMBC.2015.7318340
\item
  Wand, M., \& Schultz, T. (2011). Session-Independent EMG-Based Speech Recognition.
\item
  Yao, D., et al.~(2019). Which Reference Should We Use for EEG and ERP practice? \emph{Brain Topogr}.
\item
  Yarici, M., Thornton, M., \& Mandic, D. P. (2023). Ear-EEG sensitivity modeling for neural sources and ocular artifacts. \emph{Front Neurosci} 16:997377. doi:10.3389/fnins.2022.997377
\end{enumerate}
